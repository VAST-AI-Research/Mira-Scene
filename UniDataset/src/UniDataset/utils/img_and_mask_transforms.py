...
def check_bounding_box(bbox_w, bbox_h, min_size=None):
    if min_size is None:
        min_size = MIN_BBOX_SIZE
    if bbox_w < min_size or bbox_h < min_size:
        # raise BoundingBoxError(f"Bounding box dimensions must be at least {min_size}x{min_size}.")
        return False
    else:
        return True
...
def get_mask(
    ...
    if mask_source == "ALPHA_CHANNEL":
        if rgb_image.shape[1] != 4:
            print(f"No ALPHA CHANNEL for the image, cannot read mask.")
            mask = None
        else:
            mask = rgb_image[:, 3:4, :, :]
...
def crop_around_mask(
    ...
    """Crop image around mask bounding box with padding factor, then resize.

    Parameters
    ----------
    rgb        : [3, H, W]
    mask       : [1, H, W]
    extra_maps : list of [C, H, W] tensors to crop the same way (nearest-neighbour)

    Returns
    -------
    (cropped_rgb, cropped_mask, [cropped_extra, ...], crop_params)
    where crop_params is a dict with keys:
        top, left, bottom, right, orig_h, orig_w,
        pad_h, pad_w, pad_h_extra, pad_w_extra.
    """
    resize_bilinear = tv_transforms.Resize((target_h, target_w), antialias=True)
    resize_nearest = tv_transforms.Resize(
        (target_h, target_w),
    ...
    nonzero = mask[0].nonzero(as_tuple=False)  # [N, 2] (y, x)
    if len(nonzero) == 0:
        crop_params = {
        ...
        out = [resize_bilinear(rgb), resize_nearest(mask)]
        if extra_maps is not None:
            out += [resize_nearest(m) for m in extra_maps]
        out.append(crop_params)
        return tuple(out)
    ...
    size = max(y_max - y_min, x_max - x_min) * box_size_factor
    ...
    # Pad to square so resize does not distort aspect ratio
    crop_h, crop_w = bottom - top, right - left
    max_dim = max(crop_h, crop_w)
    pad_h = (max_dim - crop_h) // 2
    pad_w = (max_dim - crop_w) // 2
    pad_h_extra = (max_dim - crop_h) - pad_h
    pad_w_extra = (max_dim - crop_w) - pad_w
    padding = (pad_w, pad_w_extra, pad_h, pad_h_extra)

    crop_params = {
        "top": top, "left": left, "bottom": bottom, "right": right,
        "orig_h": H, "orig_w": W,
        "pad_h": pad_h, "pad_w": pad_w,
        "pad_h_extra": pad_h_extra, "pad_w_extra": pad_w_extra,
    }

    rgb_crop  = torch.nn.functional.pad(rgb_crop,  padding, mode="constant", value=0)
    mask_crop = torch.nn.functional.pad(mask_crop, padding, mode="constant", value=0)

    out = [resize_bilinear(rgb_crop), resize_nearest(mask_crop)]
    if extra_maps is not None:
        for m in extra_maps:
            m_crop = torch.nn.functional.pad(
                m[:, top:bottom, left:right], padding, mode="constant", value=0,
            )
            out.append(resize_nearest(m_crop))
    out.append(crop_params)
    return tuple(out)
...
def crop_around_mask_with_padding(
    ...
    _, bbox = compute_mask_bbox(mask, box_size_factor)
    ...
    if False:
        ...
        imageio.imwrite('./debug.png', (rgb_image.permute(1,2,0).cpu().numpy()*255).astype(np.uint8))
...
def compute_mask_bbox(
    mask: torch.Tensor, box_size_factor: float = 1.0, min_bbox_size: int = 20
    ...
    Args:
        ...
        min_bbox_size: Minimum size of the bounding box.
    ...
    is_valid = check_bounding_box(bbox_w, bbox_h, min_size=min_bbox_size)

    size = max(bbox_w, bbox_h, min_bbox_size)
...
class ObjectCentricSSI(SSIPointmapNormalizer):
    ...
    def _compute_scale_and_shift(self, pointmap: torch.Tensor, mask: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if mask_points.isfinite().max() == 0:
            ...
            print(f"No valid points found in mask; setting scale to {self.scale_factor} and shift to 0")
        ...
        # print(f"{pointmap.shape=} {mask_resized.shape=} {shift.shape=}")
