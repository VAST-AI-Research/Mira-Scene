...
def load_glb_as_zup(glb_path: str) -> trimesh.Trimesh:
    ...
    new_verts[:, 1] = -verts[:, 2]
    new_verts[:, 2] = verts[:, 1]
    return trimesh.Trimesh(vertices=new_verts, faces=np.asarray(mesh.faces), process=False)
...
class ObjaverseSceneDepthAlphaDataset(Dataset):
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.bg_mode = bg_mode  # "black", "white", or "random"
        ...
        for e in summary["entries"]:
            # Skip valid check — alpha loader uses raw RGBA renders,
            # not FLUX-replaced backgrounds, so valid flag is irrelevant
        ...
        if skipped_no_mesh > 0:
            print(
                f"ObjaverseSceneDepthAlphaDataset: skipped {skipped_no_mesh} "
        ...
        print(
            f"ObjaverseSceneDepthAlphaDataset: {len(self.entries)} entries "
    ...
    def _get_voxel(self, mesh_path: str, obj_id: str) -> Dict:
        ...
        if os.path.exists(cache_path):
            try:
                ...
                # Older cache files may not contain canonical_bbox; recompute
                # them on the fly so we never need to reload the mesh again.
                ...
                pass
        # No cache at all – load mesh and compute everything from scratch.
        mesh = self._load_mesh(mesh_path)
    ...
    def __getitem__(self, index: int) -> Dict[str, Any]:
        try:
            ...
            print(f"ObjaverseSceneDepthAlphaDataset: error at index {index}: {e}")
            traceback.print_exc()
    ...
    def _get_item(self, index: int) -> Dict[str, Any]:
        ...
        # ----------------------------------------------------------------
        # Load RGBA image and composite with background color
        # ----------------------------------------------------------------
        rgba_img = Image.open(os.path.join(scene_dir, "scene_alpha.png")).convert("RGBA")
        ...
        if self.bg_mode == "black":
            ...
            bg_color = tuple(np.random.randint(0, 256, size=3).tolist())

        bg = Image.new("RGB", rgba_img.size, bg_color)
        bg.paste(rgba_img, (0, 0), rgba_img)
        scene_img = np.array(bg)
        ...
        # with_mesh=True: mesh is still needed for output; load it explicitly.
        if self.with_mesh:
        ...
        pack = {
            ...
            "mask":          masks.float(),
            "masks":         masks.float(),
            "rgb_scene":     rgb_scene.float(),
            "rgb_cropped":   rgb_cropped.float(),
            "mask_cropped":  mask_cropped.float(),
            "fov":           fov_rad,
            "height":        self.height,
            "width":         self.width,
