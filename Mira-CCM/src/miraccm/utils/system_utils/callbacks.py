...
class CodeSnapshotCallback(VersionedCallback):
    def __init__(self, save_root, code_root=".", version=None, use_version=True):
        ...
        return [
            ...
            for b in set(
                subprocess.check_output(
                    'git ls-files -- ":!:load/*"', shell=True, cwd=self.code_root
                ).splitlines()
            )
            | set(  # hard code, TODO: use config to exclude folders or files
                subprocess.check_output(
                    "git ls-files --others --exclude-standard", shell=True, cwd=self.code_root
                ).splitlines()
            )
        ]
    ...
    def save_code_snapshot(self):
        ...
        for f in self.get_file_list():
            src_path = os.path.join(self.code_root, f)
            if not os.path.exists(src_path) or os.path.isdir(src_path):
                continue
            
            dest_path = os.path.join(self.savedir, f)
    ...
    def on_fit_start(self, trainer, pl_module):
        try:
            ...
            rank_zero_warn(
                "Code snapshot is not saved. Please make sure you have git installed and are in a git repository, and you have the right ownership."
            )
