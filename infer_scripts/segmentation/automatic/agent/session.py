...
from .core import agent_inference
...
def run_single_image_inference(
    ...
    debug=False,
    max_generations=10,
    verify_multi_selection=True,
    sam3_search_prompt=None,
    plural_membership_verifier=False,
):
    ...
    agent_history, final_output_dict, rendered_final_output = agent_inference(
        ...
        verify_multi_selection=verify_multi_selection,
        sam3_search_prompt=sam3_search_prompt,
        plural_membership_verifier=plural_membership_verifier,
    )
    print(f"{'-' * 30} End of SAM 3 Agent Session... {'-' * 30} ")

    if final_output_dict is None or rendered_final_output is None:
        print("Agent inference did not produce valid outputs. Skipping saving results.")
        return None
