...
Used by both 2_inference_CCM.py and system_ccm_voxel.py (validation_step).
"""
...
def save_ccm_outputs(
    ...
    Output files:
        ...
        rgb_ccm_cropped.png             — cropped RGB + mask + CCM rows stacked vertically
    ...
    # --- rgb_ccm_cropped.png ([RGB | mask | CCM] rows stacked vertically) ---
    _mask_crop = inp["mask_cropped_1ch"].float()
    mask_cropped_vis = ((_mask_crop > 0.5).byte() * 255)
    ...
    for i in range(NI):
        ...
        mask_i = mask_cropped_vis[i].expand(3, -1, -1).permute(1, 2, 0).cpu().numpy()
        ccm_i = ccm_cropped_vis[i].permute(1, 2, 0).cpu().numpy()
        if mask_i.shape[:2] != rgb_i.shape[:2]:
            mask_i = np.array(Image.fromarray(mask_i).resize(
                (rgb_i.shape[1], rgb_i.shape[0]), Image.NEAREST
            ))
        if ccm_i.shape[:2] != rgb_i.shape[:2]:
            ccm_i = np.array(Image.fromarray(ccm_i).resize(
                (rgb_i.shape[1], rgb_i.shape[0]), Image.NEAREST
            ))
        concat_rows.append(np.concatenate([rgb_i, mask_i, ccm_i], axis=1))
